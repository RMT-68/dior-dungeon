...
export default function GameRoom() {
   ...
   */
  const updateCharacterFromPlayer = useCallback((playerData) => {
    if (!playerData) {
      console.log("[UPDATE_CHARACTER] No player data provided");
      return;
    }

    console.log("[UPDATE_CHARACTER] Updating character with data:", {
      username: playerData.username,
      hp: playerData.current_hp,
      maxHP: playerData.character_data?.maxHP,
      stamina: playerData.current_stamina,
      maxStamina: playerData.character_data?.maxStamina,
      skills: playerData.character_data?.skills?.map((s) => s.name),
    });
  ...
  useEffect(() => {
    ...
    const handleRoomUpdate = (data) => {
      console.log(
        "[GR_ROOM_UPDATE] Received players:",
        data.players?.map((p) => ({
          id: p.id,
          username: p.username,
    ...
    const handleGameStateSync = (data) => {
      console.log("[GAME_STATE_SYNC] Received game state sync:", {
    ...
    const handleBattleResult = (data) => {
      ...
      // Handle battle status
      if (data.battleStatus === "victory") {
        addMessage("system", "🎉 Victory! Waiting for battle summary...");
        // Set game status to 'playing' so Continue button shows after battle
        setGameStatus("playing");
      } else if (data.battleStatus === "defeat") {
        addMessage("system", "💀 Defeat... The party has fallen.");
